...
const IMG_MOVIE_PATH = 'https://image.tmdb.org/t/p/w1280'  // Perbaiki URL gambar di sini
const PAGE_NUMBER = 1
...
function showMovies(movies){
    ...
    movies.forEach((movie) => {
        const {title, release_date, poster_path, vote_average, overview } = movie // vote_average typo fixed
...
function setActiveMenu(clickedElement, filter){
    var listItem = document.querySelectorAll("li a")
    listItem.forEach(function(item){
        item.classList.remove("activate");
    })

    clickedElement.classList.add("activate")
